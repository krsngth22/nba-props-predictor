...
def get_players(
    ...
    cache_key = f"players:{search}:{limit}"
    cached = get_cached(cache_key)
    if cached:
        return [PlayerResponse(**p) for p in cached]
    ...
    set_cached(cache_key, [p.model_dump() for p in players], ttl=300)
    return players
...
def get_player_stats(
    ...
    cache_key = f"stats:{player_id}:{season}:{limit}"
    cached = get_cached(cache_key)
    if cached:
        return [GameStatResponse(**s) for s in cached]
    ...
    set_cached(cache_key, [s.model_dump() for s in stats], ttl=300)
    return stats
